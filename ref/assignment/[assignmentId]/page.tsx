                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                    <div className="lg:col-span-2 space-y-4">
                        <Card className="flex flex-col space-y-4 w-full">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                <div className="flex items-center gap-2">
                                    {assignmentData.graded ? (
                                        <Badge variant="primary">Graded • /{assignmentData.maxGrade}</Badge>
                                    ) : (
                                        <Badge variant="foreground">Not graded</Badge>
                                    )}
                                </div>
                                <div className="flex items-center gap-2 text-foreground-muted">
                                    ...
                                    <span>
                                        {assignmentData.dueDate
                                            ? format(new Date(assignmentData.dueDate as unknown as string | number | Date), 'EEE, MMM d, yyyy p')
                                            : 'No due date'}
                                    </span>
                                </div>
                            </div>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                                        {assignmentData.attachments.map((attachment) => (
                                            <div className="flex flex-col rounded-md border border-border-secondary hover:bg-background-subtle transition-colors" key={attachment.id}>
                                                ...
                                                <div className="py-2 px-3 flex flex-col space-y-2">
                                                    <div className="flex flex-row space-x-2 items-center">
                                                        <span className="font-bold truncate" title={attachment.name}>{attachment.name}</span>
                                                        <Button.SM><HiDownload /></Button.SM>
                                                    </div>
                                                    <span className="text-foreground-muted text-xs truncate" title={attachment.type}>{attachment.type}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            {appState.user.teacher && (
                                <div className="flex flex-col space-y-3 pt-2 border-t border-border-secondary">
                                            ...
                                            columns={[
                                                {
                                                    ...
                                                    cell: (row: Submission) => (
                                                        <div className="flex items-center space-x-3 min-w-0">
                                                            <ProfilePicture username={row.student.username} size="md" showName={false} />
                                                            <span className="truncate">{row.student.username}</span>
                                                        </div>
                                                    ),
                                                },
                                                {
                                                    ...
                                                    cell: (row: any) => (
                                                        <span className="text-sm text-foreground">
                                                            {row.gradeReceived != null ? row.gradeReceived : '—'} / {row.assignment?.maxGrade ?? '—'}
                                                        </span>
                                                    ),
                                                },
                                                {
                                                    ...
                                                    cell: (row: Submission) => (
                                                        <span className="text-sm text-foreground">{row.attachments.length}</span>
                                                    ),
                                                },
                                                {
                                                    header: 'Status',
                                                    ...
                                                    cell: (row: Submission) => (
                                                        <div className="flex flex-wrap gap-2">
                                                            {row.late && <Badge variant="warning">Late</Badge>}
                                                            {row.submitted && !row.returned && <Badge variant="success">Submitted</Badge>}
                                                            {row.returned && <Badge variant="primary">Returned</Badge>}
                                                            {!row.submitted && !row.returned && <Badge variant="error">Missing</Badge>}
                                                        </div>
                                                    ),
                                                },
                                            ]}
                                            data={submissionsData as unknown as any[]}
                                            rowOnClick={(row: Submission) => navigation.push(ROUTES.ASSIGNMENT_SUBMISSION(params.classId, params.assignmentId, row.id))}
                                            showSelectAll={false}
                                            showActions={false}
                                            itemsPerPage={10}
                                        />
                    ...
                    {/* Sidebar (teacher) */}
                    {appState.user.teacher && <div className="lg:col-span-1 flex flex-col space-y-4">
                    <Card className="shrink-0 grow-0 flex flex-col justify-between">
                        <div className="flex flex-col space-y-2">
                    ...
                    {/* Sidebar (student) */}
                    {!appState.user.teacher && <Card className="lg:col-span-1">
